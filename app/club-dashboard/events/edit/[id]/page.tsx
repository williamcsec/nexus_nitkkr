...
import { ArrowLeft, Eye, Check, Edit2, Loader2 } from "lucide-react"
...
export default function EditEventPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const editEventWithId = editEvent.bind(null, id)
    const [state, formAction] = useActionState(editEventWithId, null)
    ...
    useEffect(() => {
        async function fetchEvent() {
            ...
            if (data) {
                ...
                setType(data.event_type || "Workshop")
                setVenue(data.venue || "")
                ...
                setMaxCapacity(String(data.max_participants || 100))
                setNPoints(String(data.n_points_reward || 0))
                setIsPaid((data.registration_fee || 0) > 0)
                setPrice(String(data.registration_fee || 0))
            }
            setLoading(false)
        }
        fetchEvent()
    ...
    return (
        <div className="space-y-6 max-w-4xl mx-auto w-full pb-20">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-2xl font-bold tracking-tight text-foreground">
                            {step === 1 ? "Edit Event" : "Review Changes"}
                        </h2>
                        <p className="text-muted-foreground">
                            {step === 1 ? "Update the details of your event" : "Preview how your event will look to students"}
                        </p>
                    </div>
                </div>
            ...
            <Card className="bg-card/50 border-border">
                <CardHeader>
                    <CardTitle>{step === 1 ? "Event Details" : "Ready to Save?"}</CardTitle>
                    <CardDescription>
                        {step === 1 ? "Provide comprehensive information to attract attendees." : "Please verify the information below before saving."}
                    </CardDescription>
                </CardHeader>
                <CardContent>
