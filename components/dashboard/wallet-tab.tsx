...
import { studentProfile, rewards, transactions, leaderboard } from "@/lib/mock-data"
      ...
      <Card className="border-primary/20 bg-gradient-to-r from-primary/10 via-card to-accent/10 overflow-hidden">
        <CardContent className="relative flex flex-col items-center gap-2 p-8 text-center sm:flex-row sm:text-left">
          ...
          <div className="sm:ml-4 flex-1">
            ...
            <p className="font-mono text-4xl font-bold text-foreground">{studentProfile.nPoints.toLocaleString()}</p>
          </div>
          <div className="flex flex-col items-center gap-1 sm:items-end">
            <Badge className="bg-primary/20 text-primary border-primary/30 gap-1">
              <TrendingUp className="h-3 w-3" /> +35 this week
            </Badge>
            <p className="text-sm text-muted-foreground">
              <Trophy className="mr-1 inline h-3.5 w-3.5 text-amber-400" />
              Rank #{studentProfile.rank} of 5,000
            </p>
          </div>
        </CardContent>
      </Card>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    className={cn(
                      ...
                      studentProfile.nPoints < reward.cost && "opacity-50 cursor-not-allowed"
                    )}
                    disabled={studentProfile.nPoints < reward.cost}
                  >
                    Redeem
                  </Button>
                </div>
        ...
        <div className="space-y-3">
          <Card className="border-border bg-card/50">
            <CardContent className="flex items-center gap-4 p-4">
              ...
              <div className="flex-1">
                <h4 className="font-semibold text-foreground">Canteen Voucher - Rs. 100</h4>
                <p className="text-xs text-muted-foreground">Redeemed on Sep 25, 2025 - Expires Dec 31, 2026</p>
              </div>
          ...
          <Card className="border-border bg-card/50 opacity-60">
            <CardContent className="flex items-center gap-4 p-4">
              ...
              <div className="flex-1">
                <h4 className="font-semibold text-foreground">NITK Nexus T-Shirt</h4>
                <p className="text-xs text-muted-foreground">Redeemed on Dec 1, 2025 - Collected</p>
              </div>
              <Badge variant="outline" className="border-border text-muted-foreground">Used</Badge>
            </CardContent>
          </Card>
        </div>
      )}
            ...
            <Card key={tx.id} className="border-border bg-card/50">
              <CardContent className="flex items-center gap-4 p-4">
                <div className={cn(
                  ...
                  tx.type === "earned" ? "bg-emerald-400/10" : "bg-red-400/10"
                )}>
                  {tx.type === "earned"
          ...
          <div className="space-y-2">
            {leaderboard.slice(3).map((entry) => (
              <Card key={entry.rank} className={cn(
                ...
                entry.name === studentProfile.name && "border-primary/30 bg-primary/5"
              )}>
