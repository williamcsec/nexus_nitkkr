...
export function ClubCard({ club }: ClubCardProps) {
  const Wrapper = club.slug ? Link : 'div' as any
  const wrapperProps = club.slug ? { href: `/clubs/${club.slug}` } : {}

  return (
    <Wrapper {...wrapperProps} className="group relative flex flex-col overflow-hidden rounded-xl border border-border bg-card/50 transition-all duration-300 hover:border-primary/30 hover:bg-card hover:shadow-lg hover:shadow-primary/5">
